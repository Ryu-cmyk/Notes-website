...
export default function PastYearPapers() {
  ...
  const handleDownload = async (url, filename, view = false) => {
    ...
    try {
      if (view) {
        // Open the actual file URL directly
        window.open(url, "_blank");
        return;
      }

      // Download with auth token if available
      const token = localStorage.getItem("access_token");
      const response = await fetch(url, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
            ...
            {data.results.map(paper => {
              ...
              return (
                <div key={paper.id} className="card" style={{ padding: "1.25rem" }}>
                  ...
                  {/* Tab switcher */}
                  {(paper.paper_files?.length > 0 || hasSolution) && (
                    <div style={{ display: "flex", borderBottom: "1px solid var(--gray-200)", marginBottom: "0.75rem" }}>
                      <button
                        onClick={() => setTab(paper.id, "paper")}
                        style={{
                          padding: "0.5rem 0.9rem",
                  ...
                  {files?.length > 0 ? (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
                      {files.map(file => (
                        <div key={file.id} style={{ display: "flex", gap: "0.25rem" }}>
                          <button
                            onClick={() => handleDownload(file.file, `${filePrefix}-${file.page_number}`, true)}
                            className="btn btn-sm"
                            style={{
                              background: tab === "solution" ? "#ECFDF5" : "var(--gray-100)",
                              color: tab === "solution" ? "var(--success)" : "var(--gray-700)",
                          ...
                          >
                            <Eye size={12} /> {tab === "paper" ? `Pg ${file.page_number}` : `Sol ${file.page_number}`}
                          </button>
                          <button
                            onClick={() => handleDownload(file.file, `${filePrefix}-${file.page_number}`, false)}
                            className="btn btn-sm"
                            style={{
                              background: "none",
                              color: tab === "solution" ? "var(--success)" : "var(--primary)",
